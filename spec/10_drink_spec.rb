...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:default_drink) { described_class.new('tequila', 1.5) }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(default_drink.type).to eq('tequila')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(default_drink.ounces).not_to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:default_drink) { described_class.new('beer', 16) }
      # remove the 'x' before running this test
      it 'is full' do
        expect(default_drink).to be_full
      end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:default_drink) { described_class.new('beer', 1) }
      # remove the 'x' before running this test
      it 'is not full' do
